PROJECT_NAME := megaport Package

SHELL            := /bin/bash
PACK             := megaport
# MITCHSTART - Need to use my github org instead of pulumi
# ORG              := pulumi
ORG              := MitchellGerdisch
# MITCHEND
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/pulumi/pulumi-megaport,${REPOSITORY},g' provider/go.mod; \
